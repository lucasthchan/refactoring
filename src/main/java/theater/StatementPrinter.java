...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    thisAmount = tragedyValue;
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += multiplier * (p.getAudience() - discount);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(),
                    frmt.format(thisAmount / divider), p.getAudience()));
            totalAmount += thisAmount;
        }
        result.append(String.format("Amount owed is %s%n", frmt.format(totalAmount / divider)));
